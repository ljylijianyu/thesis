...
# Setup the paths
dataset = Path('/home/ljy/datasets/aachen/')
images = dataset / 'images/images_upright/'

#outputs = args.outputs  # where everything will be saved
outputs = Path('/home/ljy/outputs/aachen/')
sift_sfm = outputs / 'sfm_sift'  # from which we extract the reference poses
reference_sfm = outputs / 'sfm_superpoint+superglue'  # the SfM model we will build
sfm_pairs = outputs / f'pairs-db-covis{20}.txt'  # top-k most covisible in SIFT model
loc_pairs = outputs / f'pairs-query-netvlad{50}.txt'  # top-k retrieved by NetVLAD
results = outputs / f'Aachen_hloc_superpoint+superglue_netvlad{50}.txt'
